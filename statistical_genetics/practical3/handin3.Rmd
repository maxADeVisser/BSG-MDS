...
```
Use the equation $R^2=\frac{D^2}{p_A p_B p_a p_b}$ to compute $R^2$ values. (OBS.: the above code never stopped running, so we just left here what we would have done if we had all the D statistics for each unique pair of SNPs)
```{r}
...
calc_R2(genotype_data$rs34684677, genotype_data$rs1839115, D=0.05)
```
