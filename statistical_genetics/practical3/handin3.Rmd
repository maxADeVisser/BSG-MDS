...
Given the fact that each SNP is bi-allic, there are 2 alleles per SNP. This means that there are 2^n haplotypes, where n is the number of SNPs. So there are 2^2 = 4 theoretical haplotypes.
...
```{r}
locus_labels <- list(colnames(apoe_data))
haplo.em(apoe_data, locus.label=locus_labels)
```
