...
# Using combn to generate all unique combinations of column indices
col_combinations <- combn(ncol(genotype_data), 2)
D_values <- list() # to store D_values
R_squared_values <- list() # to store R^2 values
for (i in 1:ncol(col_combinations)) {
  comb <- col_combinations[,i] # current combination
  snp1_idx <- comb[1]
  snp1 <- genotype_data[, snp1_idx]
    
  snp2_idx <- comb[2]
  snp2 <- genotype_data[, snp2_idx]
  
  D <- calc_D(genotype(snp1), genotype(snp2))
  D_values <- c(D_values, c(D)) # append to the D_values list
}

```
Use the equation $R^2=\frac{D^2}{p_A p_B p_a p_b}$ to compute $R^2$ values. (OBS.: the above code never stopped running, and we could not get it to work, so we just left here what we would have done if we had all the D statistics for each unique pair of SNPs)
```{r}
# Extract the R^2 correlation coefficient from the calculated D-statistics
calc_allele_freq <- function(snp) {
  # function to calculate allele frequency given a SNP marker
  alleles_snp <- unlist(strsplit(snp, "/"))
  alleles_counts <- table(alleles_snp)
  allele_freq <- alleles_counts / length(alleles_snp)
  return (allele_freq)
}

calc_R2 <- function(snp1, snp2, D) {
  # calculate R^2 correlation coefficient given two SNPs
  pA <- calc_allele_freq(snp1)[1] # frequency of allele 1 in snp 1
  pB <- calc_allele_freq(snp2)[1] # frequency of allele 1 in snp 1
  r2 <- D^2 / (pA * (1 - pA) * pB * (1 - pB))
  return (r2)
}

example_R2 <- calc_R2(genotype_data$rs34684677, genotype_data$rs1839115, D=0.05)
cat("Example value of a R^2 coefficient:", example_R2)
```
