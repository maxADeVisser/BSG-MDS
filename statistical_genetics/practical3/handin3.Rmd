...
for (i in 1:ncol(col_combinations)) {
  ...
  # Compute R^2 values
  alleles1 <- unlist(strsplit(snp1, "/"))
  allele_counts1 <- table(alleles1) # Count Alleles
  allele_frequencies1 <- prop.table(allele_counts1) # Calculate Frequencies
  
  alleles2 <- unlist(strsplit(snp2, "/"))
  allele_counts2 <- table(alleles2) # Count Alleles
  allele_frequencies2 <- prop.table(allele_counts2) # Calculate Frequencies
}

```
Use the equation $R^2=\frac{D^2}{p_A p_B p_a p_b}$ to extract $R^2$ values. (OBS.: the above code never stopped running, so we just left here what we would have done if we had all the D statistics for each unique pair of SNPs)
```{r}
