...
image(1:num_snps, 1:num_snps, R_squared_matrix, main="LD Heatmap (R^2)", xlab="SNP Index", ylab="SNP Index", col=my_palette)
```
...
# Creating an LD heatmap
image(1:num_snps, 1:num_snps, R_squared_matrix_filtered, main="LD Heatmap (R^2)", xlab="SNP Index", ylab="SNP Index", col=my_palette)

```

OBS.: there is no color scale attached to the heatmap, but roughly the colorscale is as such: 
- yellow: high r^2
- green: not as high as yellow, but still high
- cyan: a lower than green
- dark purple: the lowest r^2 values

When filtering out SNPs with a MAF below 0.35, it seems that the ratio of SNPs with a higher correlation coefficient is higher in comparison to when you do not (as in the first plot).
