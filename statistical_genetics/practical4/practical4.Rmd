...
```{r}
# data.frame(mds$points, best_run$points)
...
correlation_matrix[, 1:2][3:4,] # jsut to filter duplicate values
```
There is a very high positive correlation (0.9805275) between the x values of the metric and non-metric MDS. There is also very high positive correlation (0.9928759) between the x value of the metric MDS and y value of the non-metric MDS.

The values -0.04018640 and 0.03338035 indicate a very low correlation between the y dimension of metric MDS and the x and y dimensions of the non-metric MDS Since these values are close to zero, they suggest there's essentially no linear relationship between these dimensions across the two methods.
