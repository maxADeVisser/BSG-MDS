...
hist(expected_heterozygosities, main = "Expected Heterozygosity Histogram", xlab = "Expected Heterozygosity", xlim=c(0,1))
```
...
alpha <- 70
c1 <- rgb(0,0,255,max = 255, alpha = alpha)
c2 <- rgb(0,255,0,max = 255, alpha = alpha)
hist(observed_heterozygosities, col = c1, xlim=c(0,1), main="Observed vs. Expected Heterozygosities for all STRs")
hist(expected_heterozygosities, add = TRUE, col = c2, xlim=c(0,1))
legend("topright", legend = c("Observed", "Expected"), 
       fill = c(c1, c2), border = "white")
